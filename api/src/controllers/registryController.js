...
exports.createRegistry = async (req, res) => {
  try {
    const { error } = employeeRegistrySchema.validate(req.body);
    if (error) {
      return res.status(500).json({ error: error.details[0].message });
    }
    const { position_id, department_id, organization_id, employee_id } =
      req.body;
...
exports.deleteRegistry = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      'DELETE FROM employee_registry WHERE id = $1 RETURNING *',
      [id],
    );
    if (result.rows.length === 0) {
