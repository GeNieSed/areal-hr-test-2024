<template>
  ...
  <div class="content">
    ...
    <table class="styled-table">
      ...
      <tbody>
        <tr v-for="files in arrayFiles" :key="files.id">
...
export default {
  ...
  async created() {
    try {
      const response = await getOrganizations(); // Вызов API
      this.organizations = response.data; // Записываем данные в `organizations`
    } catch (error) {
