<template>
  ...
  <div class="content">
    ...
    <table class="styled-table">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.employee_registry_id">
          <td>{{ item.employee_registry_id }}</td>
          <td><img :src="`{{item.photo_path}}`" alt="Фото" width="50" /></td>
          ...
          <td>
            <button
              @click="editItem(item.employee_registry_id)"
              class="button"
              id="edit"
            >
              Редактировать
...
import { getIndex, deleteIndex } from "@/modules/index/api/indexApi";
...
const deleteItem = async (id) => {
  try {
    ...
    items.value = items.value.filter(
      (item) => item.employee_registry_id !== id,
    );
  } catch (error) {
