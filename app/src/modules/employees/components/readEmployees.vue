  ...
  <div class="content">
    ...
    <div class="function">
      <nav>
        ...
        <a href="/types" id="one">Виды операций</a>
        <a href="/employees" id="one">сссс</a>
      </nav>
      <button class="primary" @click="$router.push('/department-add')">
    ...
    <table class="styled-table">
      ...
      <tbody>
        <tr v-for="employees in arrayEmployees" :key="employees.id">
...
<script>
import { getEmployees } from "@/modules/employees/api/employeesApi";
...
export default {
  ...
  async created() {
    try {
      const response = await getEmployees(); // Вызов API
      this.arrayEmployees = response.data;
    } catch (error) {
